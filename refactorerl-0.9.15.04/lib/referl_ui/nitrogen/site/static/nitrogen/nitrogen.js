...
NitrogenClass.prototype.$replace = function(anchor, path, html) {
    if(objs(path, anchor).selector==".wfid_query_result_source_code_placeholder")
        {if(objs(path, anchor).length>0) objs(path, anchor)[0].outerHTML=html;}
    else
...
NitrogenClass.prototype.$autocomplete = function(path, autocompleteOptions, enterPostbackInfo, selectPostbackInfo) {
    ...
    jQuery.extend(autocompleteOptions, {
        select: function(ev, ui) {
          // mochijson2 can only decode these structs, when \" only appears as a string delimiter
          // JSON1 will fail, JSON2 will works
          // JSON1 = {"id": "mods[name =  \"fig\"] ", "value": "mods[name =  \"fig\"] "};
          // JSON2 = {"id": "mods[name =  \\\"fig\\\"] ", "value": "mods[name =  \\\"fig\\\"] "};
          var item = (ui.item) && '{"id":"'+ui.item.id.replace(/\"/g,"\\\"")+'","value":"'+ui.item.value.replace(/\"/g,"\\\"")+'"}' || '';
